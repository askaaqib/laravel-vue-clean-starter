<template>
  <div>
    <div v-if="exportData" class="justify-content-start">
      <b-button @click.prevent="onExportData"><i class="fe fe-download"></i> {{ $t('labels.export') }}</b-button>
    </div>
    <b-row>
      <b-col md="8" class="mb-3">
        <b-form inline v-if="search" class="d-flex" @submit.prevent>
          <b-form-input class="w-75" v-model="searchQuery" @input="debounceInput"></b-form-input>
          <b-input-group-append>
            <b-btn variant="info" @click="debounceInput">Search</b-btn>
          </b-input-group-append>
          <!-- <label class="mr-2">{{ $t('labels.datatables.search') }}</label> -->
        </b-form>
      </b-col>
      <b-col md="4" class="mb-3 d-flex justify-content-end">
        <label class="mt-2 mr-3" v-if="infos">{{ $t('labels.datatables.infos', { offset_start: perPage * (currentPage - 1) + 1, offset_end: perPage * currentPage, total: totalRows }) }}</label>
...
export default {
  ...
  methods: {
    ...
    async onBulkAction() {
      ...
      if (result.value) {
        try {
          console.log(this.actionRoute, this.selected, this.action)
